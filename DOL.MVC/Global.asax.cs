...
namespace DOL.MVC
{
    public class MvcApplication : HttpApplication
    {
        ...
        protected void Application_PostAuthenticateRequest(Object sender, EventArgs e)
        {
            ...
            using (IDataContextAsync context = new DOLDataContext())
            using (IUnitOfWorkAsync unitOfWork = new UnitOfWork(context))
            {
                //IRepositoryAsync<Account> accountRepository = new Repository<Account>(context, unitOfWork);
                IRepositoryAsync<users> accountRepository = new Repository<users>(context, unitOfWork);
                var user =
                    accountRepository.Query(u => u.login_name == username).Select().SingleOrDefault();
